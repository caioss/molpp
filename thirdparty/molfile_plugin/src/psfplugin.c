 ...
 *      $Revision: 1.84 $       $Date: 2019/09/17 00:59:04 $
 *
 ***************************************************************************/
...
static int write_psf_structure(void *v, int optflags,
  ...
  fprintf(psf->fp, psf->charmmext ? "%10d !NATOM\n" : "%8d !NATOM\n", psf->numatoms);
  ...
  for (i=0; i<psf->numatoms; i++) {
    ...
    if (psf->charmmext) {
      ...
      fprintf(psf->fp, xplorfmt ? 
                       "%10d %-8s %-8d %-8s %-8s %-6s %14.6g %13.6g %7d\n"
                     : "%10d %-8s %-8d %-8s %-8s %-4s %14.6g %13.6g %7d\n",
  ...
  if (psf->nbonds > 0 && psf->from != NULL && psf->to != NULL) {
    fprintf(psf->fp, psf->charmmext ? "%10d !NBOND: bonds\n" : "%8d !NBOND: bonds\n", psf->nbonds);
  ...
  if (psf->numangles == 0 && psf->numdihedrals == 0 && psf->numimpropers == 0 && psf->numcterms == 0) {
    ...
    fprintf(psf->fp, psf->charmmext ? "%10d !NTHETA: angles\n\n\n" : "%8d !NTHETA: angles\n\n\n", 0);
    fprintf(psf->fp, psf->charmmext ? "%10d !NPHI: dihedrals\n\n\n" : "%8d !NPHI: dihedrals\n\n\n", 0);
    fprintf(psf->fp, psf->charmmext ? "%10d !NIMPHI: impropers\n\n\n" : "%8d !NIMPHI: impropers\n\n\n", 0);
  } else {
    int i, numinline;
   ...
   */
  if (psf->charmmext) {
    fprintf(psf->fp, "%10d !NDON: donors\n\n\n", 0);
    fprintf(psf->fp, "%10d !NACC: acceptors\n\n\n", 0);
    fprintf(psf->fp, "%10d !NNB\n\n", 0);
  } else {
  ...
  if (psf->numcterms > 0) {
    fprintf(psf->fp, psf->charmmext ? "%10d !NCRTERM: cross-terms\n" : "%8d !NCRTERM: cross-terms\n", psf->numcterms);
