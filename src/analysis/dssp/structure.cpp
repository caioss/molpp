...
bool dssp::Protein::test_bond(size_t const first, size_t const second) const
{
    Residue const& first_residue = m_residues[first];
...
dssp::BridgeType dssp::Protein::test_bridge(size_t const first, size_t const second) const
{
...
double dssp::Protein::compute_h_bond(dssp::Residue& donor, dssp::Residue& acceptor)
{
    double result = 0;
...
bool dssp::Protein::no_chain_break(size_t const fisrt, size_t const last) const
{
...
double dssp::Protein::compute_kappa(size_t const index) const
{
...
bool dssp::Protein::is_last(size_t const index) const
{
    return index == m_residues.size() - 1;
}

dssp::Residue::Residue(size_t const index, size_t const original_index, mol::SSResidue const& residue)
: is_bend{false}
, sheet(0)
, structure(mol::Loop)
, m_is_proline{residue.is_proline()}
...
size_t dssp::Residue::index() const
{
    return m_index;
}

size_t dssp::Residue::external_index() const
{
    return m_external_index;
}

size_t dssp::Residue::chain() const
{
    return m_chain;
}

bool dssp::Residue::is_proline() const
{
    return m_is_proline;
}

bool dssp::Residue::is_chain_break() const
{
    return m_is_chain_break;
}

mol::Point3 const& dssp::Residue::N() const
{
    return m_N;
}

mol::Point3 const& dssp::Residue::CA() const
{
    return m_CA;
}

mol::Point3 const& dssp::Residue::C() const
{
    return m_C;
}

mol::Point3 const& dssp::Residue::O() const
{
    return m_O;
}

mol::Point3 const& dssp::Residue::H() const
{
    return m_H;
}

void dssp::Residue::set_previous(Residue const& previous)
{
    if (m_is_proline)
    {
        return;
...
bool dssp::Residue::is_valid_distance(dssp::Residue const& next) const
{
    return distance(N(), next.C()) <= MAX_PEPTIDE_BOND_LENGTH;
}

dssp::HelixType dssp::Residue::helix_flag(uint32_t const stride) const
{
    return m_helix_flags[stride - 3];
}

bool dssp::Residue::is_helix_start(uint32_t const stride) const
{
    return m_helix_flags[stride - 3] == HelixType::Start || m_helix_flags[stride - 3] == HelixType::StartAndEnd;
}

void dssp::Residue::set_helix_flag(uint32_t const stride, dssp::HelixType const type)
{
    m_helix_flags[stride - 3] = type;
}

dssp::Protein::Protein(size_t const num_residues)
{
    m_residues.reserve(num_residues);
}

void dssp::Protein::compute_secondary_structure(bool prefer_pi_helices)
{
...
std::vector<dssp::Residue> const& dssp::Protein::residues() const
{
    return m_residues;
}

void dssp::Protein::emplace_residue(size_t const external_index, mol::SSResidue const& ssresidue)
{
    size_t const index = m_residues.size();
    Residue& residue = m_residues.emplace_back(index, external_index, ssresidue);
...
void dssp::Protein::compute_helices(bool prefer_pi_helices)
{
    ...
    for (dssp::Residue& residue : m_residues)
    {
...
void dssp::Protein::compute_sheets()
{
    // Calculate Bridges
    ...
    for (uint32_t i = 0; i < bridges.size(); ++i)
    {
        for (uint32_t j = i + 1; j < bridges.size(); ++j)
        {
            ...
            if (bridges[i].type != bridges[j].type
                || !no_chain_break(std::min(ibi, ibj), std::max(iei, iej))
                || !no_chain_break(std::min(jbi, jbj), std::max(jei, jej))
                || ibj - iei >= 6
                || (iei >= ibj && ibi <= iej))
            {
                continue;
