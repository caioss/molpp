...
public:
    SelIndicesTest()
    : indices_from_list(std::to_array<mol::index_t>({1, 2, 3, 4, 5}))
    , indices_from_size{5}
    {
    }
...
// Constructor that accepts a list of indices should remove duplicates and sort them
TEST_F(SelIndicesTest, remove_duplicates_and_sort)
{
    mol::internal::SelIndices indices(std::to_array<mol::index_t>({2, 1, 3, 4, 5, 5, 4, 3, 2, 1}));
